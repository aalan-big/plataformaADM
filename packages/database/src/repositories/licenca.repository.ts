...
const includeCompleto = {
  ...
  plano:   { select: { nome: true, precoMensal: true, limiteUsuario: true, descontoTrimestral: true, descontoAnual: true } },
}
...
export async function criarLicenca(dados: {
  ...
  const chaveAtivacao = `TRIAL-${randomUUID().replace(/-/g, '').substring(0, 8).toUpperCase()}`
